...
public class DynamicAtlas : MonoBehaviour
{
    ...
    // 策略 分成格子
    private List<NxSpriteInfo> _spriteCacheList = new List<NxSpriteInfo>();
    ...
    {
        _spriteCacheList.Clear();
        for (int i = 0; i < numsX; ++i)
        {
            for (int j = 0; j < numsY; ++j)
            {
                NxSpriteInfo nxSpriteInfo = new NxSpriteInfo(i, j, _dynamicAtlasTex, i * imgWidth, j * imgHeight, imgWidth, imgHeight);
                _spriteCacheList.Add(nxSpriteInfo);
            }
        }
    }
